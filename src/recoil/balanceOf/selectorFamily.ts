...
import { _balancesAtomFamily, _balanceDenomListAtom } from './atom';

const balanceOfSelectorFamily = selectorFamily<TokenBalance, Denom>({
  key: 'balanceOf',
  get: (denom: Denom) => ({ get }) => {
    const balance = get(_balancesAtomFamily(denom));
    return balance;
  },
  set: (denom: Denom) => ({ get, set, reset }, balance) => {
